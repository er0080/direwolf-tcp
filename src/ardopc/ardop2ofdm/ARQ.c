...
void SendData()
{
	...
	case ISS:
		...
		if (bytDataToSendLength > 0)
		{
			...
			default:
				intFrameRepeatInterval = 2200;  // shouldn't get here
			}
	...
	case DISC:
		...
		if (IsCallToMe(strCallsign)) // (Handles protocol rules 1.2, 1.3)
		{
			...
			if (intReply != ConRejBW)	// If not ConRejBW the bandwidth is compatible so answer with correct ConAck frame
			{
				...
				tmrIRSPendingTimeout = Now + 15000;  // Triggers a 10 second timeout before auto abort from pending
...
#ifdef ARDOP_IP
					/* Refresh bytDataToSendLength from the TUN buffer right
					 * before deciding BREAK vs ACK.  The kernel may have
					 * queued a reply (e.g. SYN-ACK, ICMP echo reply) on the
					 * TUN since the last main-loop tick; without this call,
					 * the check below sees a stale zero and the IRS ACKs
					 * instead of breaking — the pending reply then sits in
					 * the TUN buffer indefinitely because the IRS needs
					 * ISS's cooperation to turn around the half-duplex link. */
					TUNHostPoll();
#endif
...
//				return;
			...
			if (blnFrameDecodedOK && IsDataFrame(intFrameType)) // Frame even/odd toggling will prevent duplicates in case of missed ACK
			{
				...
				bytLastACKedDataFrameLen = DataLen;
		...
		case IDLE:			// The state where the ISS has no data to send and is looking for a BREAK from the IRS
			...
			if (intFrameType == BREAK)
			{
				...
				intLinkTurnovers += 1;
				intLastARQDataFrameToHost = 0;  // precondition to an illegal frame type (ensures the new IRS does not reject a frame)
				memset(CarrierOk, 0, sizeof(CarrierOk));	// CLear MEM ARQ Stuff
